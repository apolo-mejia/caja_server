...
# Generemos el primer bloque de horas que va desde start_week hasta ese domingo
    s_iso = str(s_week.year)+'-'+str(s_week.month)+'-'+str(s_week.day)+' '+task['hf_start']
    e_iso = str(e_week.year)+'-'+str(e_week.month)+'-'+str(e_week.day)+' '+task['hf_end']
...
#        print (r_allow[ini:end])
        ...
        for i in range(28):
            if i+1 < 5 :
                ...
                if (task['d_allow'] & 1 == 1) and (t_allow[i] & 1):
                    block[j][i+1]=1
                else:
                    block[j][i+1]=0
            else:
...
# Generemos una ultima entrada a la lista bock con datos necsarios
    block.append({'n_rows':len(block),'r_allow':r_allow, 'win': win,'n_meas':win/interval, 'a_win':a_win,'s_week':s_week.strftime('%d - %m- %Y'),
                  'e_week':e_week.strftime('%d - %m- %Y'),'week':now.strftime('%W'),'h_interval':h_interval,'m_interval':m_interval})
    print (first_take)
